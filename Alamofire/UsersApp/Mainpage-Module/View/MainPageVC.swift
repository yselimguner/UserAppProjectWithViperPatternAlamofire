...
extension MainPageVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.userInfoLabel.text = "\(user.kisi_ad!) - \(user.kisi_tel!)"
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") {
            ...
            let alert = UIAlertController(title: "Delete Action", message: "Do you want to delete \(user.kisi_ad!) ?", preferredStyle: .alert)
            let cancelAction = UIAlertAction(title: "Cancel", style: .cancel){ action in
                
            }
            let yesAction = UIAlertAction(title: "Yes", style: .destructive) {action in
                self.mainPagePresenterObject?.delete(kisi_id: Int(user.kisi_id!)!)
            }
